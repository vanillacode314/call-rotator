...
import { getOutputSchema as listContactsGetOutputSchema } from '$/routes/api/v1/(protected)/list/by-path/contacts/schema';
...
export const load = (async ({ parent, params, depends, fetch }) => {
	...
	if (mode === 'offline') {
		...
		const [result1, result2] = await Promise.all([
			fetcher(nodesGetOutputSchema, '/api/v1/nodes/by-path?' + searchParams.toString()),
			fetcher(
				listContactsGetOutputSchema,
				`/api/v1/list/by-path/contacts?` + searchParams.toString()
			)
		] as const);
		if (!result1.success) {
			error(result1.status, result1.errors.map((e) => e.message).join('\n'));
		}
		if (!result2.success) {
			error(result2.status, result2.errors.map((e) => e.message).join('\n'));
		}
		node = result1.data.node;
